...
from ...helpers import get_template_path, insert_fields
from .. import image
...
if settings.PLUGINS_AND_FIELDS.get('Image'):
    ...
    class ImagePlugin(
        ...
        def get_render_template(self, context, instance, placeholder):
            return get_template_path("image", instance.template, "image")
        ...
        def get_fieldsets(self, request, obj=None):
            ...
            if type(settings.PLUGINS_AND_FIELDS.get('Image')) is dict:
                FIELDS.update(settings.PLUGINS_AND_FIELDS.get('Image'))
            fields = tuple([f for f,v in FIELDS.items() if v])
